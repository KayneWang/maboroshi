...
> Windows 用户也可直接走 [方式 1 下载预编译二进制](#windows-x86_64)；或先安装 [Rust 工具链](https://www.rust-lang.org/tools/install) 再 `cargo install --path .` / `cargo install maboroshi`。Windows 下 `maboroshi --upgrade` 为 no-op，升级请重新下载二进制，或执行 `cargo install --force maboroshi`。
