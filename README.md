...
- 🧹 **缓存自动清理** - 启动时后台检测本地音频缓存，超过大小上限自动清理最旧文件
...
> Windows 用户也可直接走 [方式 1 下载预编译二进制](#windows-x86_64)；从源码构建需要先安装 [Rust 工具链](https://www.rust-lang.org/tools/install)。`maboroshi --upgrade` 在 Windows 下为 no-op，升级请重新下载二进制，或在源码目录执行 `git pull && cargo install --path .`。
...
**自动清理**：每次启动时，程序会在后台检测缓存目录大小，超过上限（默认 500MB，可通过 `[cache]` 下的 `audio_cache_limit_mb` 配置，设为 `0` 关闭）时自动从最旧未使用的文件开始清理，并顺带清除下载中断遗留的 `.part` 文件。清理结果会显示在日志区域。

注意：**取消收藏不会自动删除对应的缓存文件**（下次自动清理时会按策略处理）。如果想立即清空，也可以手动删除缓存目录：
...
audio_cache_limit_mb = 500  # 音频缓存大小上限（MB），启动时自动清理，0 关闭
